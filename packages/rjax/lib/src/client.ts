...
export class HttpClient {
    protected handler: HttpHandler = new HttpInterceptingHandler();
    constructor(interceptors?: HttpInterceptor[]) {
        if (interceptors && interceptors instanceof Array && interceptors.length) {
    ...
    request(
        ...
        if (first instanceof HttpRequest) {
            ...
            let metas: HttpMetas | undefined = undefined;
            if (!!options.metas) {
                metas = options.metas;
            }
            // if (!!options.metas) {
            //     if (options.metas instanceof HttpMetas) {
            //         metas = options.metas;
            //     } else {
            //         metas = new HttpParams({ fromObject: options.metas });
            //     }
            // }
